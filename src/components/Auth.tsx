...
export default function Auth({ mode, onAuthSuccess, inline = false }: AuthProps) {
  ...
  const formContent = (
    <div className={`${inline ? '' : 'bg-white dark:bg-stone-900 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden p-8 relative border border-stone-100 dark:border-stone-800'}`}>
      {!inline && (
        <button 
